...
- For severe single-axis defects, keep unrelated category credit anchored to inspected evidence. A concise, valid skill that is dangerously overbroad but not unsafe usually should not fall below about `40/100` unless it also lacks a usable workflow, metadata, or output contract. A concise, valid skill with unsafe destructive instructions usually should not fall below about `20/100` unless its trigger, metadata, and basic structure are also unusable.
